...
#@ String(value="<html>Evaluate a range of values to expand segmented cells to get accurate ganglia outlines.", visibility="MESSAGE") hint4
#@ String(value="<html><b>You will need an ROI Manager with segmented cells to run this.</b>", visibility="MESSAGE") hint3
#@ File (label="Select the maximum projection or 2D image", required=false) max_proj
...
{
	if (not File.exists(max_proj)) exit("File doesn't exist or file path not provided");
